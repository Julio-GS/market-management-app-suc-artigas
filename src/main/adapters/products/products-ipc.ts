// ---------------------------------------------------------------------------
// Adapter: Products IPC handlers
//
// Electron primary adapter. Owns PRODUCTS_CHANNELS, runtime payload validation,
// handler registration/unregistration, and error mapping.
// Calls ProductService; does NOT import better-sqlite3 or call getDb().
// ---------------------------------------------------------------------------
...
export function registerProductsIpc(productService: ProductService): void {
  ipcMain.handle(
    ...
    (_event, input: unknown): OfflineProductResult => {
      ...
      try {
        return productService.createProduct(validated.data);
      } catch (err) {
  ...
  ipcMain.handle(
    ...
    (_event, productId: string, input: unknown): OfflineProductResult => {
      ...
      try {
        return productService.updateProduct(productId, validated.data);
      } catch (err) {
        if (err instanceof OfflineAuthRequiredError) {
          return { success: false, error: err.message, errorCode: "OFFLINE_AUTH_REQUIRED" };
        }
